...
function PlaylistManager() {
  ...
  } = useGetPlaylistsQuery(category);
  const [addPlaylist] = useAddPlaylistMutation();
  const [editPlaylist] = useEditPlaylistMutation();
  const { data: userPlaylistsData, refetch: refetchUserPlaylist } =
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      setError(
        "An error occurred while editing the playlist. Please try again later."
      );
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      await addPlaylist({
        body: { playlistUrl, title, description, category, userId },
      });
      ...
      setError(
        "An error occurred while adding the playlist. Please try again later."
      );
    }
  };
  ...
  const handleDelete = async (playlistId) => {
    ...
    try {
      ...
      setError(
        "An error occurred while deleting the playlist. Please try again later."
      );
    }
  };

  // Filter playlists based on their URLs
  const spotifyPlaylists = playlistsData?.playlists.filter((playlist) =>
    playlist.url.includes("open.spotify.com")
  );
  const newPlaylists = playlistsData?.playlists.filter((playlist) =>
    playlist.url.includes("open.spotify.com")
  );
  ...
  return (
    <div className="playlist-manager-container">
          ...
          {userPlaylistsData &&
            userPlaylistsData.playlists.map((playlist) => (
              <li key={playlist.id} className="playlist-item">
                ...
                <div
                  dangerouslySetInnerHTML={createMarkup(playlist.embedCode)}
                />
      ...
      <div className="new-playlists">
        ...
        <ul className="playlist-list">
          {newPlaylists &&
            newPlaylists.slice(0, 6).map((playlist) => (
              <li key={playlist.id} className="playlist-item">
                <h3>{playlist.title}</h3>
                <p>{playlist.description}</p>
                <p>
