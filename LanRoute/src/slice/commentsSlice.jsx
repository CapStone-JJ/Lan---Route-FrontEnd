...
const commentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(
        commentApi.endpoints.deleteComment.matchFulfilled,
        (state, { meta: { arg: commentId } }) => {
      ...
      .addMatcher(
        commentApi.endpoints.getVotes.matchFulfilled,
        (state, { payload }) => {
          state.votes = payload; // Update votes array with fetched votes
        }
      )
      .addMatcher(
        commentApi.endpoints.addVote.matchFulfilled,
        (state, { payload }) => {
          state.votes.push(payload); // Add new vote to votes array
        }
      )
      .addMatcher(
        commentApi.endpoints.deleteVote.matchFulfilled,
        (state, { meta: { arg: voteId } }) => {
